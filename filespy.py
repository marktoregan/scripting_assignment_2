...
parser = argparse.ArgumentParser(
            description='Watch files for certain patterns and run program on matches')
...
logger = logging.getLogger('filespy')
...
def send_alert(message, line):
    if alert_program is None:
        print("Alert: {1} [{0}]".
            format(line, message), file=sys.stderr)
    else:
